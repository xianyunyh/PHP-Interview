...
session\_start\(\) 可以加入一个数组覆盖php.ini的配置
...
#### 17. preg\_replace\_callback\_array
...
这也意味着，当发生错误的时候，以前代码中的一些错误处理的代码将无法被触发。 因为在 PHP 7 版本中，已经使用抛出异常的错误处理机制了。 （如果代码中没有捕获 **Error** 异常，那么会引发致命错误）。set\_error\_handle不一定接收的是异常，有可能是错误。
...
#### 5.$HTTP\_RAW\_POST\_DATA 被移

$HTTP\_RAW\_POST\_DATA 被移 使用`php://input`代替
...
* **PDO::PARAM\_STR\_NATL**
* **PDO::PARAM\_STR\_CHAR**
* **PDO::ATTR\_DEFAULT\_STR\_PARAM**
...
#### 1. number\_format 返回值

```php
var_dump(number_format(-0.01)); // now outputs string(1) "0" instead of string(2) "-0"
```

#### 2. get\_class\(\)不再允许null。
...
#### 8. is\_object、gettype修正

is\_object 作用在**\_\_PHP\_Incomplete\_Class** 将反正 true
